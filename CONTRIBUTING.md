# Contributing to moofetch

> **moofetch fork:** this document was inherited from upstream fastfetch. The C code style,
> build flags and module/logo architecture still apply, but upstream's project policies and
> release process do not. See [moofetch specifics](#moofetch-specifics) below for what is
...
* Build: `cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j`, or
  `./scripts/install-local.sh --no-install`. Python 3 is a build-time dependency.
* Tests (all run in CI):
  * `python3 tests/pty_smoke.py build/moofetch` — animation behaviour under a PTY
  * `node tests/web_converter.test.cjs` — browser converter logic
  * `tests/config_compat.sh build/moofetch <upstream-fastfetch>` — byte-identical output
    vs. fastfetch 2.68.1 (deterministic structures, all builtin logos, presets)
* Bundled animations live in `examples/*.anim`. Adding a file there automatically
  registers it as a builtin (CMake globs the directory) — no C changes needed. The format
  is documented in `examples/README.md`; `tools/gen-animations.py` generates the bundled
  files, and CI fails if they drift.
* Releasing: push a `vX.Y.Z` tag. `.github/workflows/release.yml` builds on Ubuntu 22.04
  with gcc-13 (C23 support while keeping older glibc) and attaches the tarball to a GitHub
  release. `.github/workflows/pages.yml` deploys `web/` to GitHub Pages.
* Keep the upstream diff reviewable: user-facing renames are fine, but internal `ff*`/`FF*`
  identifiers and file names are intentionally kept.
