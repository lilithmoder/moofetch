...
complete -c moofetch -f

function __moofetch_complete_bool
    echo -e "true\tBool"
    echo -e "false\tBool"
end

function __moofetch_complete_color
    echo -e "black\tColor"
    echo -e "red\tColor"
    echo -e "green\tColor"
    echo -e "yellow\tColor"
...
function __moofetch_complete_command
    for line in (moofetch --list-modules autocompletion)
...
function __moofetch_complete_config
    for line in (moofetch --list-presets autocompletion)
        echo -e "$line\tPreset"
    end
    echo -e "none\tDisable loading config file"
end

function __moofetch_complete_logo
...
function __moofetch_complete_structure
    for line in (moofetch --list-modules autocompletion)
        set -l pair (string split -m 2 : $line)
...
def main():
    data: dict[str, list[dict]] = json.loads(subprocess.check_output(["moofetch", "--help-raw"]))
    ...
    for key in data:
        for flag in data[key]:
            ...
            if long_name == "logo-color-[1-9]":
                ...
                for i in range(1, 10):
                    complete_commands.append(f"complete -c moofetch -d \"{safe_desc_global}\" -l \"logo-color-{i}\" -x -a \"(__moofetch_complete_color)\"")
                continue
            ...
            safe_desc_global = escape_fish_dq(flag.get("desc", ""))
            command_prefix = f"complete -c moofetch -d \"{safe_desc_global}\" -l \"{long_name}\""
            ...
            if "arg" in flag:
                ...
                if arg_type == "bool":
                    complete_commands.append(f"{command_prefix} -x -a \"(__moofetch_complete_bool)\"")
                elif arg_type == "color":
                    complete_commands.append(f"{command_prefix} -x -a \"(__moofetch_complete_color)\"")
                elif arg_type == "command":
                    complete_commands.append(f"{command_prefix} -x -a \"(__moofetch_complete_command)\"")
                elif arg_type == "config":
                    complete_commands.append(f"{command_prefix} -x -a \"(__moofetch_complete_config)\"")
                elif arg_type == "enum":
                    # Sanitize flag name to create a valid Fish function name
                    func_name = long_name.replace("-", "_").replace("[", "_").replace("]", "_")
                    func_name = f"__moofetch_complete_enum_{func_name}"
                    ...
                    complete_commands.append(f"{command_prefix} -x -a \"(__moofetch_complete_logo)\"")
                elif arg_type == "structure":
                    complete_commands.append(f"{command_prefix} -x -a \"(__fish_complete_list : __moofetch_complete_structure)\"")
                elif arg_type == "path":
